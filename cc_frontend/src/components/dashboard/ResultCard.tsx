...
export default function ResultCard({ data }: { data: any }) {
  ...
  // --- ROBUST GRAPH FIX ---
  // This automatically fixes the image source if the "data:image..." part is missing
  ...
  const ndvi = Math.round((metrics.ndvi_index || 0) * 100);
  const coverage = Math.round(metrics.vegetation_coverage || 0);
  const soil = Math.round(extra.soil_exposure || 0);
  ...
  return (
    <div className={`h-full w-full flex flex-col gap-4 p-5 bg-white font-sans overflow-y-auto transition-opacity duration-700 ${isAnimating ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="flex items-center justify-between shrink-0">
            <div>
                <h2 className="text-lg font-bold text-slate-900 tracking-tight">CropCapital Assessment</h2>
                <p className="text-xs text-slate-500 font-medium">Sentinel-2 Satellite Analysis</p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 shrink-0">
            ...
            <div className="bg-slate-900 text-white rounded-xl p-5 shadow-lg relative overflow-hidden flex flex-col justify-between h-32">
                ...
                <div className="flex justify-between items-end border-t border-white/10 pt-2">
                    <div className="text-xs font-medium text-emerald-400">{tier.split(':')[0]}</div>
                    <div className="text-[10px] text-slate-400">{score > 700 ? "Low Risk" : "High Risk"}</div>
                </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-100 flex flex-col justify-between h-32">
                 <div>
                    ...
                    <div className="text-2xl font-semibold text-slate-900 mt-1">{total_limit}</div>
                 </div>
                 
                 <div className="space-y-1">
                     <div className="flex justify-between text-[10px] text-slate-500">
                        <span>Eligibility</span>
                        <span className="font-bold text-emerald-600">{eligibility}%</span>
                     </div>
        ...
        <div className="shrink-0">
           <p className="text-[10px] font-bold text-slate-800 mb-3 uppercase tracking-wide">Satellite Metrics</p>
           <div className="grid grid-cols-3 gap-3">
              <MetricCircle percent={ndvi} label="NDVI" color="#10b981" />
              <MetricCircle percent={coverage} label="Cover" color="#3b82f6" />
              <MetricCircle percent={soil} label="Soil" color="#f59e0b" />
           </div>
        </div>

        {/* RBI GRAPH (With Flex Grow to fill space) */}
        {graphSource ? (
            <div className="flex-1 min-h-0 border-t border-slate-100 pt-3 flex flex-col">
                <div className="flex justify-between items-center mb-2 shrink-0">
                    <p className="text-[10px] font-bold text-slate-800 uppercase tracking-wide">Loan Structure</p>
                    <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded border border-slate-200">RBI Norms</span>
                </div>
                <div className="flex-1 border border-slate-100 rounded-xl bg-white shadow-sm flex justify-center items-center overflow-hidden p-2 relative">
                    <img 
                        src={graphSource} 
                        alt="KCC Breakdown" 
